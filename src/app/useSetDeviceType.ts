...
export const useSetDeviceType = () => {
	...
	useEffect(() => {
		if (window.innerWidth <= mobileWidth && deviceType !== 'mobile') dispatch(setDeviceType({ deviceType: 'mobile' }));
		if (window.innerWidth > mobileWidth && deviceType !== 'desktop') dispatch(setDeviceType({ deviceType: 'desktop' }));
